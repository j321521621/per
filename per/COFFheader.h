...
public:
    ...
    DWORD Init(PCHAR base,DWORD size)
    {
        ...
        assert(GetPetype()!=PETYPE_EXCUTE || m_PointerToSymbolTable==0,L"m_PointerToSymbolTable非法(=0)无法初始化COFFHeader",FALSE);
        assert(GetPetype()!=PETYPE_EXCUTE || m_NumberOfSymbols==0,L"m_NumberOfSymbols非法(=0)无法初始化COFFHeader",FALSE);
        assert(GetPetype()!=PETYPE_OBJECT || m_SizeOfOptionalHeader==0,L"m_SizeOfOptionalHeader非法(=0)无法初始化COFFHeader",FALSE);
    ...
    void Print()
    {
        ...
        wprintf(L"标志位    : [%8x]\n",m_Characteristics);
        wprintfc(((m_Characteristics&IMAGE_FILE_RELOCS_STRIPPED)?HWIHTE:DWIHTE),L"        不含重定位信息\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_EXECUTABLE_IMAGE)?HWIHTE:DWIHTE),L"        可运行\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_LINE_NUMS_STRIPPED)?HWIHTE:DWIHTE),L"        不含源码信息\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_LOCAL_SYMS_STRIPPED)?HWIHTE:DWIHTE),L"        不含符号信息\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_LARGE_ADDRESS_AWARE)?HWIHTE:DWIHTE),L"        大内存感知\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_BYTES_REVERSED_LO)?HWIHTE:DWIHTE),L"        小端机器\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_32BIT_MACHINE)?HWIHTE:DWIHTE),L"        32位程序\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_DEBUG_STRIPPED)?HWIHTE:DWIHTE),L"        不含调试信息\n",);
        ...
        wprintfc(((m_Characteristics&IMAGE_FILE_DLL)?HWIHTE:DWIHTE),L"        DLL文件\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_UP_SYSTEM_ONLY)?HWIHTE:DWIHTE),L"        限制单处理器\n",);
        wprintfc(((m_Characteristics&IMAGE_FILE_BYTES_REVERSED_LO)?HWIHTE:DWIHTE),L"        大端机器\n",);
    }
